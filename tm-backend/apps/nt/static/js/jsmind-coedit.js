...
(function($w) {
	...
	jm.prototype = {
		...
		set_edit_info: function(nodeid, editor, time) {
			if(this.get_editable()) {
				var node = this.mind.get_node(nodeid);
				if(!!node) {
					if(!!editor) {
			...
			if(!!node.topic) {
				if(this.opts.support_html) {
					...
					d.innerHTML = "<div class='mindMarkBox' style='height:" + markNodeHeight + "px'>" + remarkNode + filemarkNode + "</div>" +
						"<p class='nodeContent' style='margin:0px'>" + node.topic + "</p>";
		...
		edit_node_end: function() {
			if(this.editing_node != null) {
				...
				if(jm.util.text.is_empty(topic) || node.topic === topic) {
					...
					console.log('开始更新！');
					var operatons = {};
					operatons['action']='update';
					operatons['upid']=node.id;
					operatons['topic']=topic;
					var daynow = new Date();
					var timenow = daynow.Format("yyyy-MM-dd HH:mm:ss");
					operatons['editor']=username.value;
					operatons['edittime']=timenow;
					ws.send(JSON.stringify(operatons));
					this.jm.set_edit_info(node.id, username.value,timenow);
					console.log('成功更新！');
				}
			}
